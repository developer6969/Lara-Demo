	...
	<div class="mt-4">
		...
		<div class="row">
			...
			<div class="col-md-3">
                ...
                <!-- Clinics -->
				<div class="card card-outline card-info" v-show="showClinicsList">
                    ...
                    <!-- Clinic List Recycler View -->
					<div class="card-body table-responsive p-0">
						<table class="table table-hover">
							<tbody class="recycler-view-table">
								<tr
									class="recycler-view-item pl-1"
									v-bind:class="getActiveClinic(clinic.id)"
									v-for="clinic in clinics"
									:key="clinic.id"
									@click="viewClinicDetail(clinic)"
								>
									<img :src="'/img/' + clinic.photo" class="profile-image-thumb img-circle mr-2" style="float: left">
                                    <p class="m-0 pt-2"><strong>{{ clinic.name }}</strong></p>
                                    <p class="mp-0"><small><em>{{ clinic.shortname }}</em></small></p>                             
								</tr>
							</tbody>
						</table>
					</div>
				...
				<div class="card card-outline card-info" v-show="!showClinicsList">
					...
					<div class="card-header pl-2 pb-0">
						<h3 class="card-title">
                            <span>
                                <button type="button" class="btn btn-sm btn-inline mp-0" @click="showClinicsList = !showClinicsList">
                                    <i class="fas fa-chevron-left pl-2 pr-2" style="font-size: 1.2rem !important; color: #595959"></i>
                                </button>
                            </span>
                            Doctors
                        </h3>
						<div class="card-tools">
                            ...
                            <button type="button" class="btn btn-secondary btn-sm" @click="createUserModal">
                             ...
                             <!-- Close Doctor View -->
                            <!-- <button type="button" class="btn btn-secondary btn-sm" @click="showClinicDetail = !showClinicDetail">
            ...
            <!-- Show Selection -->
            <div class="col-md-9" v-show="showProfileView">

                <!-- Clinic's Profile -->
                <div class="row" v-show="showClinicProfile">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="card card-outline card-warning">
                            <div class="card-header mb-0 pb-0">
                                <h3 class="card-title">{{ aclinic.name }}</h3>
                                <div class="card-tools">
                                    <button type="button" class="btn btn-tool" @click="closeClinicProfile()">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-4">
                                        <img class="img-responsive" width="100%" :src="'img/' + aclinic.photo" alt="Clinic Image" style="object-fit: fill">
                                    </div>
                                    <div class="col-md-8">
                                        <p class="text-justify" v-if="showAboutTextFull">
                                            {{ aclinic.about }}
                                            <span>
                                                <a class="float-link" href="#" @click="showText()"><small>less</small></a>
                                            </span>
                                        </p>
                                        <p class="text-justify" v-else>
                                            {{ aclinic.about | truncate(250) }}
                                            <span v-if="textIsLong(aclinic.about)">
                                                <a class="float-link" href="#" @click="showText()"><small>more</small></a>
                                            </span>
                                        </p>
                                    </div>
                                </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <ul class="list-inline mp-0">
                                    <li class="list-inline-item" 
                                    v-for="facility in facilities" :key="facility.id"
                                    style="border: 1px solid grey; padding: 2px 6px !important; 
                                           border-radius: 25px; margin-bottom: 8px !important;
                        ...
                        <!-- General Information -->
                         <div class="card">
                            <div class="card-body">
                                <p class="text-secondary text-sm mb-0">For Enquery Contact</p>
                                <p class="mt-0">{{ aclinic.incharge }} at  {{ aclinic.phone }}</p>

                                <p class="text-secondary text-sm mb-0">Working Hours</p>
                                <p class="mb-0">Open on all days<span v-show="aclinic.holiday"> except {{ aclinic.holiday }}</span></p>
                                <p class="mb-0">From {{ aclinic.opentime }} hrs till {{ aclinic.closetime }} hrs</p>
                                <p class="mt-0" v-show="aclinic.lunchtime">Lunch at : {{ aclinic.lunchtime }} hrs for {{ aclinic.lunchduration }} min</p>
                                <p class="text-secondary text-sm mb-0" v-show="aclinic.lunchtime">Address</p>
                                <p class="text-secondary text-sm mb-0 mt-2" v-show="!aclinic.lunchtime">Address</p>
                                <p class="mt-0 mb-0">{{ aclinic.number }}, {{ aclinic.street }}</p>
                                <p class="mt-0 mb-0">{{ aclinic.area }}, {{ aclinic.city }}</p>
                                <p class="mt-0 mb-0">{{ aclinic.state }}, {{ aclinic.country }} - {{ aclinic.pincode }}.</p>
                                <p class="mt-0">Lmk - {{ aclinic.landmark }}</p>
                                <iframe :src="aclinic.map" width="100%" height="auto" frameborder="0" style="border:0;" allowfullscreen=""></iframe>
                            </div>
                        </div>
                ...
                <!-- Doctor's Profile -->
                <div class="row" v-show="showDoctorProfile">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            <p class="text-justify" v-else>
                                                {{ doctor.about | truncate(250) }}
                                                <span v-if="textIsLong(doctor.about)">
                                                    <a class="float-link" href="#" @click="showText()"><small>more</small></a>
                                                </span>
                                            </p>
                                        </div>
			...
			<!-- No selected -->
			<div class="col-md-9" v-show="!showProfileView" style="height: 83vh; pink; position: relative; border: 1px solid #b2b2b2 !important; border-radius: 5px;">
...
export default {
  ...
  methods: {
    ...
    viewClinicDetail(clickedItem) {
        this.showProfileView = true;
        this.showClinicsList = false; // means clinic list hides and doctors list became visible
        this.showClinicProfile = true; // means clinic profile becames visible
        this.showDetailUser = true;
        this.activeClinicId = clickedItem.id;
        this.aclinic = clickedItem;
        this.clinicdoc = clickedItem.clinicdoc;
        this.facilities = clickedItem.facility;
    },
    closeClinicProfile() {

    },
    isDoctorAvaialbelAtClinic(user) {
        if (user.role == 'doctor') {
            for (let index = 0; index < Object.keys(this.clinicdoc).length; index++) {
                if(user.doctor.id == this.clinicdoc[index].doctor_id) {
                    return true;
                }
            } 
        }
    ...
    getActiveClinic(clickedItemId) {
        if (this.activeClinicId === clickedItemId) {
            return "activeMember";
        }
    },
    orderedClinic() {
        const array = this.clinics;
        let sampleValues = [];
        for (let index = 0; index < Object.keys(this.clinics).length; index++) {
            sampleValues.push(array[index].clinic_id);
        }
